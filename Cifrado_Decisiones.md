...
usuarios fichando por PIN o reconocimiento facial. Maneja dos archivos con datos sensibles, en
el almacenamiento interno privado de la app (`filesDir` → `/data/data/com.example.iclock/files/`):
...
El proyecto era **offline**. Está decidido que las fichadas **se sincronizarán a un backend
existente**: el servidor pasa a ser la **fuente de verdad** y el equipo, un **buzón temporal (outbox)**.
...
**Por qué:** activar el cifrado sin migrar dejaría ilegibles (o perdería) los datos ya grabados. Tras la
integración (D14), al leer un archivo viejo en claro el descifrado falla y devuelve vacío → los usuarios
enrolados "desaparecen" hasta migrar o re-enrolar.
...
| 2026-06-10 | `MainActivity.kt` | Integración: las 4 funciones de E/S ahora delegan en los repos (se quitó `writeText`/`appendText`/`readText` en claro y `calcularDistancia`). **La app ya cifra de verdad.** | D14 | #6 |

---

## Pendientes / requisitos externos

- **Migración (tarea #7):** IMPORTANTE hacerla antes de correr con datos reales. Tras la integración, los
  archivos viejos en texto plano ya no se leen (el descifrado falla → lista vacía). Para probar en limpio:
  borrar los datos de la app. Para no perder lo existente: implementar la migración.
